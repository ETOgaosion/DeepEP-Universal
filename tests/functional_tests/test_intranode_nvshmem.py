...
def _resolve_nvshmem_tokens(args: argparse.Namespace, num_tokens: int) -> tuple[int, int]:
    send_tokens = args.num_max_nvshmem_chunked_send_tokens or 6
    recv_tokens = args.num_max_nvshmem_chunked_recv_tokens or max(256, num_tokens)
    if recv_tokens <= send_tokens:
        recv_tokens = send_tokens + 1
    return send_tokens, recv_tokens


def _compute_nvshmem_bytes(args: argparse.Namespace, hidden: int, num_ranks: int, num_tokens: int) -> int:
    send_tokens, recv_tokens = _resolve_nvshmem_tokens(args, num_tokens)
    ...
    return max(args.nvshmem_bytes, int(config.get_nvl_buffer_size_hint(hidden_bytes, num_ranks)))
...
def _run_all_to_all(args: argparse.Namespace, local_rank: int, num_ranks: int, rank: int,
    ...
    if local_rank == 0:
        print("[ok] intranode NVSHMEM all-to-all dispatch + combine passed", flush=True)


def test_loop(local_rank: int, num_local_ranks: int, args: argparse.Namespace):
    rank, num_ranks, group = init_dist(local_rank, num_local_ranks)
    nvshmem_bytes = _compute_nvshmem_bytes(args, args.hidden, num_ranks, args.num_tokens)
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Minimal intranode NVSHMEM all-to-all test")
    ...
    parser.add_argument("--num-max-nvshmem-chunked-send-tokens", type=int, default=6,
                        help="Max NVSHMEM chunked send tokens (default: 6)")
    parser.add_argument("--num-max-nvshmem-chunked-recv-tokens", type=int, default=0,
                        help="Max NVSHMEM chunked recv tokens (default: auto)")
    parser.add_argument("--nvshmem-bytes", type=int, default=0,
                        help="Override NVSHMEM buffer bytes; 0 uses the computed minimum")
